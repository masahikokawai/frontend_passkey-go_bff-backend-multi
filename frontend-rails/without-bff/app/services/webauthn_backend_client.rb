...
# CONTRACT.mdセクション22.4/22.9: backendのパスキー内部API
# (/internal/v1/auth/webauthn/credentials/*)を共有シークレットヘッダ(X-Webauthn-Internal-Token)で
# 呼ぶ専用クライアント。パスキーでのログイン試行中はまだセッション/access tokenが無い
# (ログイン処理そのもの)ため、BackendClient(ユーザー自身のaccess tokenで呼ぶ)とは別に用意する
# (bff-rails/app/services/webauthn_backend_client.rbと全く同じ設計・実装)
