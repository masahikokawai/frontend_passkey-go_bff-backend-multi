...
// backend-js: CONTRACT.mdセクション20のTask CRUD JavaScript実装(Express + @grpc/grpc-js)。
// backend(Go)と同一のワイヤー契約(REST JSON形状・gRPC .proto)を持つことが最重要の制約。
// 1プロセスでREST・gRPC・外部公開APIの3つを同時に起動する構成もbackend(Go)のcmd/server/main.go・
// backend-rustのsrc/main.rsに倣った。
...
const db = require('./db');
const { Dispatcher, HmacVerifier, JwksVerifier, LOCAL_HMAC_ISSUER, LOCAL_RSA_ISSUER } = require('./auth/jwt');
const { FlagCache } = require('./flags');
const restModule = require('./rest');
const externalModule = require('./external');
