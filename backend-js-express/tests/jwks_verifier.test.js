...
// backend-java/JwksVerifierTest.java・backend-python/test_jwks_verifier.pyの移植。
// 自プロセス内蔵のモックJWKSサーバーを使い、実際のRS256署名検証・kidキャッシュ・
// 未知kid時の再取得ロジックを検証する。DB・Keycloak・bff実プロセスは不要
const test = require('node:test');
const assert = require('node:assert/strict');
const { JwksVerifier, VerifyError } = require('../src/auth/jwt');
const { createMockJwksServer } = require('./support/mock_jwks_server');
const { generateRsaKeyPair, makeRsaToken } = require('./support/test_token_helper');
