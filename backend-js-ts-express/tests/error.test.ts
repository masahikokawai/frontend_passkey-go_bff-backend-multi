// backend-js/tests/error.test.jsの型付き移植。テスト内容は変更していない。

import test from 'node:test';
import assert from 'node:assert/strict';
import * as errors from '../src/error';

// backend(Go)・backend-rust・backend-jsのエラー分岐と1対1で対応することを確認する
test('error factories produce the exact contract status/body shape', () => {
  const cases: Array<[errors.RestError, number, Record<string, unknown>]> = [
