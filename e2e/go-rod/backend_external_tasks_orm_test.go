...
func setExternalTasksOrm(t *testing.T, variation string) {
	...
	// (e2e/chromedpのwaitForFeatureFlagPropagationと同じ待ち時間)
...
func fetchExternalAPIToken(t *testing.T) string {
	...
	tokenURL := getEnv("KEYCLOAK_TOKEN_URL", "http://localhost:8082/realms/training/protocol/openid-connect/token")
	resp, err := http.PostForm(tokenURL, url.Values{
		"grant_type":    {"client_credentials"},
		"client_id":     {getEnv("EXTERNAL_API_CLIENT_ID", "external-api-client")},
		"client_secret": {getEnv("EXTERNAL_API_CLIENT_SECRET", "external-api-client-local-dev-secret")},
	})
	if err != nil {
...
func externalTasksBaseURL() string {
	return getEnv("BACKEND_EXTERNAL_API_BASE_URL", "http://localhost:8097")
}
...
// (e2e/chromedpの同名テストと全く同じ内容、フレームワーク間のGoコードの重複は
// このプロジェクトの既存の慣例通り許容する)
